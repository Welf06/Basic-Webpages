...
//eye
let eye1 = document.querySelectorAll(".eye")[0];
let eye2 = document.querySelectorAll(".eye")[1];
...
window.onmousemove=function(event){
    ...
    var y = event.clientY;
    console.log(y + " " + x);
    if(y >= 227 && y < 300 && x >= 740 && x <= 810){

        eye1.style.transform= `translate(0px , 49px )`;
        eye2.style.transform= `translate(0px , 7px )`;

    }else{
       
        y= y * 0.0263 ;
        x= x * 0.03 ;
            eye1.style.transform= `translate(${19-y}px , ${x}px )`;
            eye2.style.transform= `translate(${19-y}px , ${x}px )`;
...
// TODO: update the below hardcoded values with ones depending on the pupil/ eye size
function computeEyeCoordinate(mouseCoordinate, eyeCenterCoordinate) {
    return (
        7 + (8 * (mouseCoordinate - eyeCenterCoordinate)) / eyeCenterCoordinate
    );
}
